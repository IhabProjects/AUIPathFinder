...
import { sbaMinorQuestions } from '../data/questions';
import QuestionSlider from '../components/QuestionSlider';

export default function MinorQuiz() {
  ...
  useEffect(() => {
    if (!schoolMatch?.name || schoolMatch.name !== 'SBA') {
  ...
  const question = sbaMinorQuestions[currentQuestion];
  ...
  const handleNext = () => {
    if (currentQuestion < sbaMinorQuestions.length - 1) {
    ...
    <div className="max-w-3xl mx-auto">
      <div className="bg-white rounded-lg shadow-notion p-8 mb-8">
        ...
        <div className="relative w-full bg-gray-200 h-2 rounded-full">
          <div
            ...
            style={{ width: `${((currentQuestion + 1) / sbaMinorQuestions.length) * 100}%` }}
          />
        </div>
        <p className="text-gray-600 mt-2">
          Question {currentQuestion + 1} of {sbaMinorQuestions.length}
        </p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-notion p-6">
        ...
        <div className="flex justify-between items-center">
          ...
          >
            {currentQuestion === sbaMinorQuestions.length - 1 ? 'See Results' : 'Next Question'}
          </button>
        </div>
      </div>
    </div>
